...
class CudaGraphRunner:
    ...
    def __init__(self, model_runner: "ModelRunner"):
        ...
        if model_runner.server_args.disable_cuda_graph_padding:
            ...
            self.capture_bs = [1, 2, 4] + [i * 8 for i in range(1, 21)] # 1, 2, 4, 8, 16,（+8递增）,160
        # 过滤掉预设数据里超过范围的部分
        self.capture_bs = [
            bs
            ...
            for bs in self.capture_bs:
                with patch_model(
                    ...
                    (
                        graph,
                        output_buffers,
                    ) = self.capture_one_batch_size(bs, forward) # 针对每个bs，会构建一个graph，每个graph的cuda stream以及输入输出buffer首地址是同一份
                    self.graphs[bs] = graph                      
                    self.output_buffers[bs] = output_buffers     # 每个graph，都会有对应的输出buffer（bs不同，buffer大小也不同）
    ...
    def replay(self, forward_batch: ForwardBatch):
        ...
        if forward_batch.return_logprob:
            ...
            # LogitsProcessorOutput 只是将相关结果数据打包在一起再输出
