...
# 
# 注意cuda graph里面无法执行python原生的print，因为graph replay里面都需要是cuda操作。
...
# 
# F.linear相当于 return torch.matmul(x, layer.weight.t()) + bias。
# 对于该linear文件中实现的线性层，非量化实现最底层都是调用这个pytorch的linear实现。
#
# quant_method由quant_config得到，quant_config会从模型定义层的时候传入
...
class QKVParallelLinear(ColumnParallelLinear):
    ...
    def __init__(
		...
		#      假设有4个kv_head, 每个kv_head对应4个q_head, 共16个q_head.        #      如果有4个节点，则每个节点一人一个，q也刚好能整除, 每个节点有4个q_head。下面的num_kv_heads和num_kv_head_replicas都为1.
        ...
        #      * self.head_size是q_head和kv_head都共用的，即所有head的head_size都一致。对应权重输出维度N.
...
class RowParallelLinear(LinearBase):
	...
	# 否则需要调用split_tensor_along_last_dim，切分一下。默认需要规约，叠加汇总结果
