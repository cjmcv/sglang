...
# <NT> 因果mask仅在prefill和extend阶段需要额外加，而在decode阶段不需要。
...
def _fwd_kernel_stage1(
    ...
    # <NT> 在extend中，一个线程block会取到q的多行，因为这几行有可能会是同一个序列的，需要做gemm。
...
# <NT> 以GQA为例，当group_num等于head_num时，就退化成了MHA。group是针对KV cache而言的，而head_num指的是查询头，即是q的head_num。
# 
