...
def _fwd_kernel(
	...
	# head_dim方向上的mask，因为BLOCK_DMODEL是由next_power_of_2(LK)得到的，实际超出范围部分需要舍弃掉。
    mask_d = offs_d < Lk
    ...
    for start_n in range(0, block_mask * end_n, BLOCK_N):
    	# tl.multiple_of()是将start_n调整为BLOCK_N的倍数。本身就是倍数，可能是多一个保护？
...
def context_attention_fwd(
	...
	# triton.cdiv是向上取整的除法, BLOCK是一个block的负责的数据大小, 表示z方向需要多少个block来处理这些数据。
