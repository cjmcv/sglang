...
if [ "$2" = "0" ]; then
    if [ "$1" = "sglang" ]; then
        ...
        $NSYS_PROFILER python3 -m sglang.launch_server --model-path $MODEL_PATH --enable-torch-compile --disable-radix-cache --enable-mixed-chunk 
    elif [ "$1" = "vllm" ]; then
...
# lighteval vllm     "pretrained=/home/cjmcv/project/llm_models/Qwen/Qwen2___5-1___5B-Instruct,dtype=float16"     "leaderboard|truthfulqa:mc|0|0"